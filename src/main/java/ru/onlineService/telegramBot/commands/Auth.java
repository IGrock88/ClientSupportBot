...
public class Auth extends BaseCommand {
    ...
    public Auth(ChatHandler chatHandler, TelegramBot bot) {
    ...
    public void execute(Update update) {
        ...
        if (message.length < 2) {
            bot.execute(new SendMessage(chatId, "Не введен пароль. Введите " + AUTH + " и через пробел пароль."));
            return;
        }
        if (!message[1].equals(Constants.botPass)) {
            bot.execute(new SendMessage(chatId, "Пароль не верный"));
            return;
        }
        if (chatHandler.getChat(chatId) != null) {
            bot.execute(new SendMessage(chatId, "Вы уже авторизованы"));
            return;
        }
        ...
        bot.execute(new SendMessage(chatId, "Вход выполнен. Спасибо!"));
